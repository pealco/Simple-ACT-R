...
compute.base.levels = function(moment) {
    ...
    # the decay function
    tjd = tj ^ -d
    decay = matrix(tjd[tj>0], nrow=trials, ncol=num.past.retrievals, byrow=T)
...
# computes distribution of retrieval times for all items given a retrieval cue 
# feature vector and a moment in time to do the retrieval.  Updates the 
# history matrix with the winning items for this retrieval moment over all the 
# trials.
